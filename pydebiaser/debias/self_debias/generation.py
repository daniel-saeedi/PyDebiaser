...
class SelfDebiasingT5ForConditionalGeneration(
    T5ForConditionalGeneration, GenerationMixin
):
    """
    ...
    def beam_sample(self, *args, **kwargs):
        raise NotImplementedError(
            "Beam sampling is not implemented for class SelfDebiasingT5ForConditionalGeneration"
        )

    def sample(self, *args, **kwargs):
        raise NotImplementedError(
            "Sampling is not implemented for class SelfDebiasingT5ForConditionalGeneration"
        )
        ...
        while cur_len < max_length:
            ...
            if self.logits_processor is not None:
                batch_size = next_tokens.shape[0] // (
                    1 + self.logits_processor.num_debiasing_prefixes
                )
                regular_sentence_indices = range(batch_size)
                for regular_sentence_idx in regular_sentence_indices:
                    debiasing_sentence_indices = (
                        self.logits_processor._get_bias_indices(
                            regular_sentence_idx, batch_size
                        )
                    )
                    for debiasing_sentence_idx in debiasing_sentence_indices:
                        next_tokens[debiasing_sentence_idx] = next_tokens[
...
class SelfDebiasing(GenerationMixin):
    """
    This class represents a regular AutoModelForCausalLM that additionally has the capacity to perform self-debiasing. For self-debiasing, the
    init_logits_processor function must be called. Otherwise, this model just performs regular language modeling.
    """
    ...
    def beam_sample(self, *args, **kwargs):
        raise NotImplementedError(
            "Beam sampling is not implemented for class SelfDebiasingAutoModelForCausalLM"
        )
