...
class ModelArguments:
    ...
    model: str = field(
        ...
        metadata={
            "help": "Model to load. These correspond to a HuggingFace class (e.g., BertForSequenceClassification)."
        },
    )
...
def main():
    ...
    # Load the model.
    kwargs = {}
    if model_args.bias_direction is not None:
        # Load the pre-computed bias direction for SentenceDebias.
        bias_direction = torch.load(model_args.bias_direction)
        kwargs["bias_direction"] = bias_direction

    if model_args.projection_matrix is not None:
        # Load the pre-computed projection matrix for INLP.
        projection_matrix = torch.load(model_args.projection_matrix)
